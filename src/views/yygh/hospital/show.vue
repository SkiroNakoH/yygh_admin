    ...
    <div class="app-container">
        ...
        <table class="table table-striped table-condenseda table-bordered" width="100%">
            <tbody>
                <tr>
                    <th width="15%">医院名称</th>
                    <td width="35%"><b style="font-size: 14px">{{ hospital.hosname }}</b> | {{ hospital.param.hostypeString
                    }}</td>
...
export default {
    ...
    methods: {
        //根据id查询医院详情
        fetachHospDetail(id) {
            hospital.getHospById(id)
                .then(response => {
                    this.hospital = response.data.hospital
                    this.bookingRule = response.data.hospital.bookingRule
                    //加载logo
                    this.logo = "data:image/jpeg;base64," + this.hospital.logoData;
                })
        },
        //返回医院列表
        back() {
            this.$router.push("/yygh/hospital/list")
        }
    }
}
</script>